...
export function ReviewDetailPage() {
  ...
  return <section className="review-page stage-review">
    ...
    <Card className="card-stack"><h2>{t('review.original')}</h2><p className="long-copy">{creative.koreanText}</p><h2>{t('review.latestZh')}</h2><p className="long-copy">{latestLocalization?.text ?? t('review.none')}</p>{latestLocalization?.koBackTranslation && <div className="localized-box"><span className="facet-label">{t('review.backTranslation')}</span><p className="long-copy">{latestLocalization.koBackTranslation}</p></div>}</Card>
