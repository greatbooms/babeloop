...
| # | 슬라이스 | 내용 | 완료 기준 (E2E) |
|---|---|---|---|
| 0 | 골격 | 모노레포, Docker Compose, 인증, 브랜드/시장 등록, `ai_execution_logs` | 로그인 → 브랜드 등록 → 목록 표시 |
| 1 | 업로드 파이프라인 | Presigned 업로드, MediaAsset, BullMQ, Mock OCR/전사, Job 폴링 | 이미지 업로드 → 분석 완료 → OCR 결과 표시 |
| 2 | 분석·검색 | 광고 분석(Mock), 임베딩, pgvector 유사 검색 | 광고 2개 등록 → 유사 광고 검색 결과 표시 |
| 3 | 생성 | 브리프 → 문구·스크립트 변형 → zh-TW 초안 | 브리프 생성 → 변형 3개 → zh-TW 초안 표시 |
| 4 | 검토·내보내기 | 상태 머신, 검토 큐, BabeGuard 기본, 실험/변형, 추적코드, 파일 내보내기 | 검수 → 승인 → 추적코드 각인 패키지 다운로드 |
| 5 | 성과 | CSV 업로드, 퍼널 대시보드, 그레인 표시, 브리프 환류 | CSV 업로드 → 소재별 퍼널 표시 → 환류 브리프 생성 |
